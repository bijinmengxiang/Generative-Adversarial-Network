...
for i in range(3):
    for data,target in train_loader:
        ...
        model.train(data.cuda(),torch.FloatTensor([1.0]).cuda())
        #model.train(generate_randomdata(784),torch.zeros(1))
        #第二步：训练鉴别器，给鉴别器喂生成器产生的数据    
        model.train(Generator.forward(generate_randomdata_seed(100).cuda()).detach(),torch.FloatTensor([0.0]))
        #第三步：训练生成器，将假数据喂给鉴别器后，保存loss，用此loss来训练生成器
        Generator.train(model,generate_randomdata_seed(100).cuda(),torch.FloatTensor([1.0]).cuda())
...
for i in range(2):
    for j in range(3):
        output = Generator.forward(generate_randomdata_seed(100).cuda())
